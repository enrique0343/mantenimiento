...
export const POST: APIRoute = async (ctx) => {
  ...
  const { tipo, cantidad, motivo, notas } = parsed.data;
  ...
  // Stock actual (bodega única, una sola fila por item)
  const [actual] = await db.select().from(stock).where(eq(stock.itemId, itemId)).limit(1);
  ...
  else nuevaCantidad = cantidad; // ajuste = absoluta
  ...
  if (actual) {
    ...
    await db.insert(stock).values({ itemId, cantidad: nuevaCantidad });
  }
